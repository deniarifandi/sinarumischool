...
public function whatsappWebhook()
{
    ...
    if ($messageType === 'location') {
        ...
        // -----------------------------------------------------
        // DELETE OLD LOCATION FOR THIS PHONE
        // -----------------------------------------------------

        $db->table('whatsapp_locations')
            ->where('phone', $phone)
            ->delete();
        ...
        $db->table('whatsapp_locations')->insert([
            'phone'     => $phone,
            'latitude'  => $latitude,
            'longitude' => $longitude,
        ...
        try {
            ...
            log_message(
                'error',
                'WA LOCATION LOG ERROR: ' . $e->getMessage()
            );
        }

        log_message(
            'info',
            'WA LOCATION RESPONSE: ' .
    ...
    $location = $db->table('whatsapp_locations')
        ->where('phone', $phone)
    ...
    if ($location) {
        ...
        // Location maximum age: 30 minutes
        $locationTime = strtotime($location['created_at']);
        $currentTime  = time();

        if (($currentTime - $locationTime) <= 1800) {
    ...
    if (!$latitude || !$longitude) {
        ...
        $responseText =
            ...
            "Silakan kirim lokasi Anda melalui WhatsApp terlebih dahulu.\n\n" .
            "Setelah itu ketik:\n" .
            "absen\n" .
            "izin\n" .
            "sakit";
    ...
    public function whatsappWebhookFIX()
{
    $db = \Config\Database::connect();
